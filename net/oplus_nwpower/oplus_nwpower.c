...
extern void (*match_qrtr_wakeup)(int src_node, int src_port, int dst_port,
...
static void tcp_output_tcpsynretrans_hook_work_callback(
...
DECLARE_WORK(tcp_output_tcpsynretrans_hook_work,
	     tcp_output_tcpsynretrans_hook_work_callback);
DECLARE_WORK(tcp_input_tcpsynretrans_hook_work,
	     tcp_input_tcpsynretrans_hook_work_callback);
...
static void nwpower_unsl_blacklist_reject()
{
	if (record_blacklist_reject_index > 0) {
		...
		nwpower_send_to_user(NW_POWER_REPORT_BLACK_REJECT, (char *)blacklist_reject_uid,
				     sizeof(blacklist_reject_uid));
		memset(blacklist_reject_uid, 0x0,
		       OPLUS_MAX_RECORD_BLACK_REJECT_LEN * sizeof(u32));
...
extern bool oplus_check_socket_in_blacklist(int is_input, struct socket *sock)
{
	...
	if (blacklist_len > 0 && sock && sock->sk && (sock->sk->sk_family == 2
			|| sock->sk->sk_family == 10)) {
		...
		for (i = 0; i < blacklist_len; i++) {
			if (blacklist_uid[i] == uid) {
				if (is_input != 1) {
					...
					if (record_blacklist_reject_index < OPLUS_MAX_RECORD_BLACK_REJECT_LEN) {
						blacklist_reject_uid[record_blacklist_reject_index * 2 + 1] = now_ts.tv_sec *
								1000 + now_ts.tv_nsec / 1000000;
						blacklist_reject_uid[record_blacklist_reject_index * 2 + 2] =
							(u64)uid << 32 | is_input;
...
static void __oplus_match_qrtr_wakeup(int src_node, int src_port, int dst_port,
				      unsigned int arg1, unsigned int arg2, u64 qrtr[][4],
				      u64 wakeup[OPLUS_NW_WAKEUP_SUM], bool prt)
{
	int i;
	int repeat[4] = {0};
	int repeat_index = 0;
		...
		if (repeat_index == 1) {
			...
			if (prt)
				printk("[oplus_nwpower] QrtrWakeup: ServiceID: %d, NodeID: %d, PortID: %d, Msg: [%08x %08x], Count: %d",
				       qrtr[repeat[0]][1], src_node, qrtr[repeat[0]][2], arg1, arg2,
				       qrtr[repeat[0]][3]);
			...
			if (prt)
				printk("[oplus_nwpower] QrtrWakeup: ServiceID: [%d/%d/%d/%d], NodeID: %d, PortID: %d, Msg: [%08x %08x], Count: %d",
				       ...
				       src_node, qrtr[repeat[repeat_index - 1]][2], arg1, arg2,
				       qrtr[repeat[repeat_index - 1]][3]);
...
static void oplus_match_qrtr_wakeup(int src_node, int src_port, int dst_port,
				    unsigned int arg1, unsigned int arg2)
{
	__oplus_match_qrtr_wakeup(src_node, src_port, dst_port, arg1, arg2,
				  service_wakeup_times, oplus_nw_wakeup, true);
	__oplus_match_qrtr_wakeup(src_node, src_port, dst_port, arg1, arg2,
				  mdaci_service_wakeup_times, oplus_mdaci_nw_wakeup, false);
...
static void print_qrtr_wakeup(bool unsl, u64 qrtr[][4],
			      u64 wakeup[OPLUS_NW_WAKEUP_SUM], bool prt)
{
	u64 temp[5][4] = {{0}};
	u64 max_count = 0;
	...
	for (j = 0; j < 5; ++j) {
		...
		if (unsl) {
			if (temp[j][3] > 0) wakeup_unsl_msg[j] = temp[j][2] << 32 | ((
							u32)temp[j][1] << 16 | (u16)temp[j][3]);
		}
	...
	if (prt)
		printk("[oplus_nwpower] AllWakeups: Mpss: %d, Qrtr: %d, Modem: %d, WiFi: %d",
		       wakeup[OPLUS_NW_MPSS], wakeup[OPLUS_NW_QRTR], wakeup[OPLUS_NW_MD],
		       wakeup[OPLUS_NW_WIFI]);
}
	...
	if (atomic_read(&ipa_wakeup_hook_boot) == 1) {
		if (atomic_read(&tcp_is_input) == 0) {
			...
			if (((now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000) -
					(tcp_last_transmission_stamp.tv_sec * 1000 +
					 tcp_last_transmission_stamp.tv_nsec / 1000000)) > OPLUS_TRANSMISSION_INTERVAL) {
				if (type == OPLUS_TCP_TYPE_V4) {
					...
					tcp_input_list.ipv6_addr1 = (u64)ntohl(tmp_v6iph->saddr.s6_addr32[0]) << 32 |
								    ntohl(tmp_v6iph->saddr.s6_addr32[1]);
					tcp_input_list.ipv6_addr2 = (u64)ntohl(tmp_v6iph->saddr.s6_addr32[2]) << 32 |
								    ntohl(tmp_v6iph->saddr.s6_addr32[3]);
...
static void oplus_match_ipa_tcp_wakeup(int type, struct sock *sk)
{
	if (atomic_read(&ipa_wakeup_hook_boot) == 1) {
		...
		sk->oplus_last_rcv_stamp[1] = tcp_last_transmission_stamp.tv_sec * 1000 +
					      tcp_last_transmission_stamp.tv_nsec / 1000000;
	}
}

static void oplus_ipa_schedule_work(void)
{
	if (atomic_read(&ipa_wakeup_hook_boot) == 1 && atomic_read(&tcp_is_input) == 1
			&& !tcp_input_sch_work) {
		schedule_work(&tcp_input_hook_work);
		tcp_input_sch_work = true;
	}
...
static void oplus_match_tcp_output(struct sock *sk)
{
	...
	if (atomic_read(&ipa_wakeup_hook_boot) == 1) {
		if (atomic_read(&tcp_is_input) == 0) {
			...
			if (((now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000) -
				...
				if (sk->sk_v6_daddr.s6_addr32[0] == 0 && sk->sk_v6_daddr.s6_addr32[1] == 0) {
					...
					tcp_output_list.ipv6_addr1 = (u64)ntohl(sk->sk_v6_daddr.s6_addr32[0]) << 32 |
								     ntohl(sk->sk_v6_daddr.s6_addr32[1]);
					tcp_output_list.ipv6_addr2 = (u64)ntohl(sk->sk_v6_daddr.s6_addr32[2]) << 32 |
		...
		sk->oplus_last_send_stamp[1] = tcp_last_transmission_stamp.tv_sec * 1000 +
					       tcp_last_transmission_stamp.tv_nsec / 1000000;
	}
}
...
static void oplus_match_tcp_input_retrans(struct sock *sk)
{
	...
	if (atomic_read(&tcpsynretrans_hook_boot) == 1) {
		...
		if (((now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000) -
				sk->oplus_last_rcv_stamp[0]) > OPLUS_TCP_RETRANSMISSION_INTERVAL) {
			if (sk->sk_v6_daddr.s6_addr32[0] == 0 && sk->sk_v6_daddr.s6_addr32[1] == 0) {
				...
				tcp_input_retrans_list.ipv6_addr1 = (u64)ntohl(sk->sk_v6_daddr.s6_addr32[0]) <<
								    32 | ntohl(sk->sk_v6_daddr.s6_addr32[1]);
				tcp_input_retrans_list.ipv6_addr2 = (u64)ntohl(sk->sk_v6_daddr.s6_addr32[2]) <<
								    32 | ntohl(sk->sk_v6_daddr.s6_addr32[3]);
...
static void oplus_match_tcp_output_retrans(struct sock *sk)
{
	...
	if (atomic_read(&tcpsynretrans_hook_boot) == 1) {
		...
		if (((now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000) -
				sk->oplus_last_send_stamp[0]) > OPLUS_TCP_RETRANSMISSION_INTERVAL) {
			if (sk->sk_v6_daddr.s6_addr32[0] == 0 && sk->sk_v6_daddr.s6_addr32[1] == 0) {
				...
				tcp_output_retrans_list.ipv6_addr1 = (u64)ntohl(sk->sk_v6_daddr.s6_addr32[0]) <<
								     32 | ntohl(sk->sk_v6_daddr.s6_addr32[1]);
				tcp_output_retrans_list.ipv6_addr2 = (u64)ntohl(sk->sk_v6_daddr.s6_addr32[2]) <<
								     32 | ntohl(sk->sk_v6_daddr.s6_addr32[3]);
...
{
	nwpower_send_to_user(NW_POWER_REPORT_APP_WAKEUP,
			     (char *)app_wakeup_monitor_list.set, sizeof(app_wakeup_monitor_list.set));
	app_wakeup_monitor_list.count = 0;
	memset(app_wakeup_monitor_list.set, 0x0, sizeof(app_wakeup_monitor_list.set));
}


static void app_wakeup_monitor(struct tcp_hook_simple_struct *pval,
			       bool is_block, bool is_input, int pid, int uid)
{
	...
	if (pval->count < OPLUS_MAX_RECORD_APP_WAKEUP_LEN) {
		...
		pval->set[1 + pval->count * 3 + 1] =
			...
			(0xFFFF);
		pval->set[1 + pval->count * 3 + 2] = now_ts.tv_sec * 1000 + now_ts.tv_nsec /
						     1000000;
...
static void tcp_output_hook_work_callback(struct work_struct *work)
{
	...
	app_wakeup_monitor(&app_wakeup_monitor_list, false, false, tcp_output_list.pid,
			   tcp_output_list.uid);
	app_wakeup_monitor(&mdaci_app_wakeup_monitor_list, false, false,
			   tcp_output_list.pid, tcp_output_list.uid);

	if (tcp_output_list.is_ipv6) {
		printk("[oplus_nwpower] IPAOutputWakeup: [%ld,****], %d, %d, %d",
		       tcp_output_list.ipv6_addr1,
		       tcp_output_list.pid, tcp_output_list.uid,
		       tcp_output_list.set[3 * i + 2] & 0xFFFFFFFF);
...
static void tcp_input_hook_work_callback(struct work_struct *work)
{
	...
	app_wakeup_monitor(&app_wakeup_monitor_list, false, true, tcp_input_list.pid,
			   tcp_input_list.uid);
	app_wakeup_monitor(&mdaci_app_wakeup_monitor_list, false, true,
			   tcp_input_list.pid, tcp_input_list.uid);
	...
	if (tcp_input_list.is_ipv6) {
		printk("[oplus_nwpower] IPAInputWakeup: [%ld,****], %d, %d, %d",
		       ...
		       tcp_input_list.pid, tcp_input_list.uid,
		       tcp_input_list.set[3 * i + 2] & 0xFFFFFFFF);
	...
	struct work_struct *work)
{
	...
	if (tcp_output_retrans_list.is_ipv6) {
		if (tcp_output_retrans_list.ipv6_addr1 == 0
				&& tcp_output_retrans_list.ipv6_addr2 == 0)
			return;

		printk("[oplus_nwpower] TCPOutputRetrans: [%ld,****], %d, %d, %d",
		       tcp_output_retrans_list.ipv6_addr1,
		       tcp_output_retrans_list.pid, tcp_output_retrans_list.uid,
		       tcp_output_retrans_list.set[3 * i + 2] & 0xFFFFFFFF);
		...
		printk("[oplus_nwpower] TCPOutputRetrans: %#X, %d, %d, %d",
		       tcp_output_retrans_list.ipv4_addr & 0xFFFFFF, tcp_output_retrans_list.pid,
		       tcp_output_retrans_list.uid,
...
static void tcp_input_tcpsynretrans_hook_work_callback(struct work_struct *work)
{
	...
	if (tcp_input_retrans_list.is_ipv6) {
		if (tcp_input_retrans_list.ipv6_addr1 == 0
				&& tcp_input_retrans_list.ipv6_addr2 == 0)
			return;

		printk("[oplus_nwpower] TCPInputRetrans: [%ld,****], %d, %d, %d",
		       tcp_input_retrans_list.ipv6_addr1,
		       tcp_input_retrans_list.pid, tcp_input_retrans_list.uid,
		       tcp_input_retrans_list.set[3 * i + 2] & 0xFFFFFFFF);
		...
		printk("[oplus_nwpower] TCPInputRetrans: %#X, %d, %d, %d",
		       tcp_input_retrans_list.ipv4_addr & 0xFFFFFF, tcp_input_retrans_list.pid,
		       tcp_input_retrans_list.uid,
		       (tcp_input_retrans_list.set[3 * i + 2] & 0xFFFC000000000000) >> 50);
	}
}

static int print_tcp_wakeup(const char *type, struct tcp_hook_struct *pval,
			    bool prt)
{
	int i;
	u32 count;
...
static void print_ipa_wakeup(bool unsl, struct tcp_hook_struct *ptcp_in,
			     struct tcp_hook_struct *ptcp_out, struct tcp_hook_struct *ptcp_re_in,
			     struct tcp_hook_struct *ptcp_re_out, u64 wakeup[OPLUS_NW_WAKEUP_SUM], bool prt)
{
	...
	wakeup[OPLUS_NW_TCP_RE_OUT] = print_tcp_wakeup("OutputRetrans", ptcp_re_out,
				      prt);

	if (prt)
		printk("[oplus_nwpower] IPAAllWakeups: TCPInput: %d, TCPOutput: %d, TCPInputRetrans: %d, TCPOutputRetrans: %d",
		       wakeup[OPLUS_NW_TCP_IN], wakeup[OPLUS_NW_TCP_OUT], wakeup[OPLUS_NW_TCP_RE_IN],
		       wakeup[OPLUS_NW_TCP_RE_OUT]);
...
static void reset_count(u64 qrtr[][4], struct tcp_hook_struct *ptcp_in,
			struct tcp_hook_struct *ptcp_out,
			struct tcp_hook_struct *ptcp_re_in, struct tcp_hook_struct *ptcp_re_out,
			u64 wakeup[OPLUS_NW_WAKEUP_SUM])
{
	int i;
	int j;
...
static void nwpower_hook_off(bool unsl)
{
	...
	print_ipa_wakeup(unsl, &tcp_input_list, &tcp_output_list,
			 &tcp_input_retrans_list, &tcp_output_retrans_list, oplus_nw_wakeup, true);
	reset_count(service_wakeup_times, &tcp_input_list, &tcp_output_list,
		    &tcp_input_retrans_list, &tcp_output_retrans_list, oplus_nw_wakeup);
	app_wakeup_monitor_list.count = 0;
	memset(app_wakeup_monitor_list.set, 0x0, sizeof(app_wakeup_monitor_list.set));

	if (unsl) {
		nwpower_send_to_user(NW_POWER_UNSL_MONITOR, (char *)wakeup_unsl_msg,
				     sizeof(wakeup_unsl_msg));
...
{
	print_qrtr_wakeup(true, mdaci_service_wakeup_times, oplus_mdaci_nw_wakeup,
			  true);
	print_ipa_wakeup(true, &mdaci_tcp_input_list, &mdaci_tcp_output_list,
			 &mdaci_tcp_input_retrans_list, &mdaci_tcp_output_retrans_list,
			 oplus_mdaci_nw_wakeup, true);
	reset_count(mdaci_service_wakeup_times, &mdaci_tcp_input_list,
		    &mdaci_tcp_output_list,
		    &mdaci_tcp_input_retrans_list, &mdaci_tcp_output_retrans_list,
		    oplus_mdaci_nw_wakeup);
	nwpower_send_to_user(NW_POWER_REPORT_MDACI, (char *)wakeup_unsl_msg,
			     sizeof(wakeup_unsl_msg));
	memset(wakeup_unsl_msg, 0x0, sizeof(wakeup_unsl_msg));
	nwpower_send_to_user(NW_POWER_REPORT_MDACI_APP_WAKEUP,
			     (char *)mdaci_app_wakeup_monitor_list.set,
			     sizeof(mdaci_app_wakeup_monitor_list.set));
	mdaci_app_wakeup_monitor_list.count = 0;
	memset(mdaci_app_wakeup_monitor_list.set, 0x0,
	       sizeof(mdaci_app_wakeup_monitor_list.set));
}
...
static int nwpower_netlink_init(void)
{
	...
	oplus_nwpower_sock = netlink_kernel_create(&init_net,
			     NETLINK_OPLUS_NWPOWERSTATE, &cfg);
