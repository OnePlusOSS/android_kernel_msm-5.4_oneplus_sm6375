...
static int oplus_score_netlink_rcv_msg(struct sk_buff *skb,
...
static int oplus_score_send_netlink_msg(int msg_type, char *payload,
					int payload_len);
...
static int oplus_update_score_count(int link, int score, int flag)
{
	...
	if (flag) {
		...
		if (((score > WORST_BASE_SCORE)
				&& (uplink_score_info[link].uplink_score_count >= 0)) ||
				((score <= WORST_BASE_SCORE)
				 && (uplink_score_info[link].uplink_score_count <= 0)))
			ret = OPLUS_TRUE;
		...
		if (((score <= WORST_BASE_SCORE)
				&& (downlink_score_info[link].downlink_score_count <= 0)) ||
				((score > WORST_BASE_SCORE)
				 && (downlink_score_info[link].downlink_score_count >= 0)))
			ret = OPLUS_TRUE;
...
void oplus_score_calc_and_report(void)
{
	...
	for (i = 0; i < MAX_LINK_NUM; i++) {
		...
		if (uplink_total_packets == 0) {
			...
			if (uplink_packets > ((para_rate * PACKET_PER_SEC) >> 3))
			...
			else if (uplink_srtt > BAD_GAME_RTT_THREAD && (uplink_total_packets < 10)
					&& (downlink_total_packets < 10))
				uplink_score = (s32)(WORST_BASE_SCORE - (WORST_BASE_SCORE * retrans_rate *
						     para_loss) / 800);
				...
				if (uplink_srtt > WORST_VIDEO_RTT_THRESH)
					...
					if (downlink_rate == 0)
						...
						uplink_score = (s32)(GOOD_BASE_SCORE - (GOOD_BASE_SCORE * retrans_rate *
											para_loss) / 800);

					else if (downlink_rate > PACKET_PER_SEC)
						uplink_score = (s32)(NORMAL_BASE_SCORE - (NORMAL_BASE_SCORE * retrans_rate *
								     para_loss) / 800);
					...
					uplink_score = (s32)(WORSE_BASE_SCORE - (WORSE_BASE_SCORE * retrans_rate *
							     para_loss) / 800);
		...
		if (oplus_score_debug) {
			printk("[oplus_score]:up_score:link=%u,if=%s,up_pack=%u,up_retran=%u,up_rtt=%u,score=%d,s_score=%d,seq=%u,uid=%u,retrans_rate=%u,index=%u,nodata=%u\n",
			       uplink_index, ifname, uplink_packets, uplink_retrans_packets, uplink_srtt,
			       uplink_score,
			       uplink_smooth_score, uplink_seq, oplus_score_foreground_uid[0], retrans_rate,
			       index, uplink_nodata_count);
		}
		...
		if (downlink_packets + downlink_retrans_packets == 0) {
			...
			if (downlink_packets > ((para_rate * PACKET_PER_SEC) >> 3))
				...
				if (uplink_srtt > WORST_RTT_THRESH)
					downlink_score = (s32)(WORST_BASE_SCORE - (WORST_BASE_SCORE * retrans_rate *
							       para_loss) / 800);
		...
		if (oplus_score_debug) {
			printk("[oplus_score]:down_score:link=%u,if=%s,down_pack=%u,down_retran=%u,down_rtt=%u,score=%d,s_score=%d,seq=%u,uid=%u,retrans_rate=%u,index=%u\n",
			       downlink_index, ifname, downlink_packets, downlink_retrans_packets,
			       downlink_srtt, downlink_score,
			       downlink_smooth_score, downlink_seq, oplus_score_foreground_uid[0],
			       retrans_rate, index);
		}
		...
		if (uplink_report || downlink_report) {
			...
			oplus_score_send_netlink_msg(OPLUS_SCORE_MSG_REPORT_NETWORK_SCORE,
						     (char *)&link_score_msg, sizeof(link_score_msg));

			if (oplus_score_debug) {
				printk("[oplus_score]:report_score1:link=%u,if=%s,up_score=%d,down_score=%d,uid=%u,ul_p=%d,dl_p=%d\n",
				       uplink_index, ifname, link_score_msg.uplink_score,
				       link_score_msg.downlink_score,
		...
		if (oplus_score_debug) {
			printk("[oplus_score]:report_score_all:link=%u,uplink_score=%d,us_score=%d,downlink_score=%d,ds_score=%d,uid=%u,ul_p=%d,dl_p=%d\n",
			       ...
			       downlink_smooth_score, oplus_score_foreground_uid[0], uplink_report,
			       downlink_report);
		}
	}
					...
					int payload_len)
{
	...
	ret = genl_msg_prepare_usr_msg(OPLUS_SCORE_CMD_DOWNLINK, size,
				       oplus_score_user_pid, &skbuff);
...
static void oplus_score_uplink_stat(struct sk_buff *skb)
{
	...
	if (i < 0) {
		if (oplus_score_debug) {
			printk("[oplus_score]:upskb_linkerr,link=%d,if=%s,seq=%u,ack=%u,sport=%u,dport=%u,uid=%u\n",
			       link_index, skb->dev->name, ntohl(tcph->seq), ntohl(tcph->ack_seq),
			       ntohs(tcph->source),
	...
	if (icsk->icsk_ca_state >= TCP_CA_Recovery && tp->high_seq != 0
			&& before(ntohl(tcph->seq), tp->high_seq))
	...
	if (oplus_score_debug) {
		printk("[oplus_score]:uplink=%d,if=%s,seq=%u,high_seq=%u,uplink_retran=%u,npacket=%u,uid=%u,sport=%u,dport=%u,state=%d,len=%u\n",
		       link_index, skb->dev->name, ntohl(tcph->seq), tp->high_seq,
		       uplink_score_info[i].uplink_retrans_packets,
	...
	if (srtt > VALID_RTT_THRESH) {
		...
		if (uplink_score_info[i].uplink_rtt_num != 0) {
			uplink_score_info[i].uplink_srtt =
				(uplink_score_info[i].uplink_srtt * (uplink_score_info[i].uplink_rtt_num - 1) +
				 srtt) / uplink_score_info[i].uplink_rtt_num;
...
static void oplus_score_downlink_stat(struct sk_buff *skb)
{
	...
	if ((sk->sk_state != TCP_SYN_SENT)
			&& (is_downlink_retrans_pack(ntohl(tcph->seq), sk)))
	...
	if (oplus_score_debug) {
		printk("[oplus_score]:downlink=%d,if=%s,seq=%u,rcv_nxt=%u,downlink_retran=%u,npacket=%u,uid=%u,sport=%u,dport=%u,state=%d,len=%u\n",
		       link_index, skb->dev->name, ntohl(tcph->seq), tp->rcv_nxt,
		       downlink_score_info[i].downlink_retrans_packets,
		       downlink_score_info[i].downlink_packets, (u32)(sk->sk_uid.val),
		       ntohs(tcph->source),
		       ntohs(tcph->dest), sk->sk_state, skb->len);
	}
	...
	if (srtt) {
		...
		if (downlink_score_info[i].downlink_rtt_num != 0) {
			downlink_score_info[i].downlink_srtt =
				(downlink_score_info[i].downlink_srtt * (downlink_score_info[i].downlink_rtt_num
...
static inline int skb_v4_check(struct sk_buff *skb)
{
	...
	/* skb is pure ack*/
	if ((ntohs(iph ->tot_len) == (iph->ihl + tcph->doff) * 4) && (!tcph->syn
			|| !tcph->fin))
		return OPLUS_FALSE;
...
static inline int skb_v6_check(struct sk_buff *skb)
{
	...
	/* skb is pure ack*/
	if ((ntohs(ipv6h ->payload_len) == tcph->doff * 4) && (!tcph->syn
			|| !tcph->fin))
		return OPLUS_FALSE;
...
static unsigned int oplus_score_postrouting_hook4(void *priv,
...
static unsigned int oplus_score_postrouting_hook6(void *priv,
		struct sk_buff *skb, const struct nf_hook_state *state)
{
	if (!oplus_score_enable_flag)
		return NF_ACCEPT;
...
static unsigned int oplus_score_input_hook4(void *priv, struct sk_buff *skb,
...
static unsigned int oplus_score_input_hook6(void *priv, struct sk_buff *skb,
		const struct nf_hook_state *state)
{
	if (!oplus_score_enable_flag)
		return NF_ACCEPT;
...
static void oplus_score_request_score(struct nlattr *nla)
{
	...
	link_score_msg.downlink_score = oplus_get_smooth_score(link_index,
					OPLUS_DOWNLINK);
	spin_unlock_bh(&downlink_score_lock);

	link_score_msg.link_index = link_index;
	printk("[oplus_score]:request_report:link=%d,uscore=%d,dscore=%d\n", link_index,
	       link_score_msg.uplink_score, link_score_msg.downlink_score);
	oplus_score_send_netlink_msg(OPLUS_SCORE_MSG_REPORT_NETWORK_SCORE,
				     (char *)&link_score_msg, sizeof(link_score_msg));
	return;
}
...
static void oplus_score_add_uplink(u32 link_index, struct net_device *dev)
{
	...
	for (i = 0; i < MAX_LINK_NUM; i++) {
		...
		printk("[oplus_score]:up:add_link finish,link_index=%u,i=%d,link_num=%u",
		       link_index, i, oplus_score_uplink_num);
		break;
	}
...
static void oplus_score_add_downlink(u32 link_index, struct net_device *dev)
{
	...
	for (i = 0; i < MAX_LINK_NUM; i++) {
		...
		printk("[oplus_score]:down:add_link finish,link_index=%u,i=%d,link_num=%u",
		       link_index, i, oplus_score_downlink_num);
		break;
	}
}
...
static int proc_set_test_foreground_uid(struct ctl_table *ctl, int write,
...
static int proc_set_test_link_index(struct ctl_table *ctl, int write,
				    void __user *buffer, size_t *lenp, loff_t *ppos)
{
	int ret;
	u32 data[2];
...
{
	oplus_score_table_hrd = register_net_sysctl(&init_net, "net/oplus_score",
				oplus_score_sysctl_table);
...
static int __init oplus_score_init(void)
{
	...
	ret = nf_register_net_hooks(&init_net, oplus_score_netfilter_ops,
				    ARRAY_SIZE(oplus_score_netfilter_ops));
...
{
	oplus_score_netlink_exit();
	nf_unregister_net_hooks(&init_net, oplus_score_netfilter_ops,
				ARRAY_SIZE(oplus_score_netfilter_ops));
