...
extern int (*handle_dhcp)(struct sock *sk, struct sk_buff *skb,
...
static int oplus_dhcp_send_to_user(int msg_type, char *payload, int payload_len)
{
	...
	if (oplus_dhcp_pid) {
		...
		printk(KERN_ERR
		       "oplus_dhcp_hooks_netlink: can not unicast skbuff, oplus_dhcp_pid=0\n");
		kfree_skb(skbuff);
	}
...
static void send_dhcp_offer_packet_to_userspace(char *wlan_iface,
		__be32 server_addr, char *server_mac)
{
	struct notify_offer payload;
	int index = MAIN_WLAN_INDEX;

	debug("%s, %u.%u.%u.***, %02x:%02x:***:***:%02x:%02x\n", wlan_iface,
	      IP_MASK(server_addr), MAC_MASK(server_mac));

	if (wlan_iface != NULL) {
		if (0 == memcmp(wlan_iface, MAIN_WLAN_IFACE, strlen(MAIN_WLAN_IFACE))) {
			index = MAIN_WLAN_INDEX;

		} else if (0 == memcmp(wlan_iface, SECOND_WLAN_IFACE,
				       strlen(SECOND_WLAN_IFACE))) {
	...
	oplus_dhcp_send_to_user(OPLUS_DHCP_NOTIFY_DUP_OFFER_EVENT, (char *)&payload,
				sizeof(payload));
}
...
static void handle_dhcp_packet(void *data, struct sock *sk, struct sk_buff *skb,
			       struct net_device *dev, struct packet_type *pt, int *do_drop)
{
	struct dhcp_pkt *b;
	struct iphdr *iph;
	int len, ext_len;
...
{
	oplus_dhcp_hooks_table_hrd = register_net_sysctl(&init_net,
				     "net/oplus_dhcp_hooks",
