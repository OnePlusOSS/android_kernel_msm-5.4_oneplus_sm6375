...
static int goodix_get_chip_info(void *chip_data)
{
	...
	reg_info->GTP_REG_DETAILED_DEBUG_INFO   = 0x3096;    /*debug_reg*/
...
static void goodix_detailed_debug_info_read(struct seq_file *s, void *chip_data, debug_type debug_type)
{
	...
	ret = goodix_get_channel_num(chip_info->client, &tx_num, &tx_num);
	if (ret < 0) {
	...
	for(i = 0; i < rx_num; i++) {
		seq_printf(s, "[%2d] ", i);
		for(j = 0; j < tx_num; j++) {
			data = (kernel_buf[j * tx_num * 2 + i * 2] << 8) + kernel_buf[j * rx_num * 2 + i * 2 + 1];
