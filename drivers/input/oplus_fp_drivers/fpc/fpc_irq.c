...
int fpc_power_on(struct fpc1020_data *fpc_dev)
{
	...
	for (index = 0; index < fpc_dev->power_num; index++) {
		...
		case FP_POWER_MODE_GPIO:
			gpio_set_value(fpc_dev->pwr_list[index].pwr_gpio,
...
static ssize_t hardware_reset(struct device *dev,
			      struct device_attribute *attribute, const char *buffer, size_t count)
{
...
static DEVICE_ATTR(irq_enable, S_IRUSR | S_IWUSR, irq_enable_get,
		   irq_enable_set);
...
static int fpc1020_irq_probe(struct platform_device *pldev)
{
	...
	rc = of_property_read_u32(np, "fpc,use_gpio_power_enable",
				  &g_use_gpio_power_enable);
