...
static void silfp_hw_reset(struct silfp_data *fp_dev, u8 delay)
{
	...
	if (fp_dev->rst_port > 0) {
		...
		if (fp_dev->irq_no_use && fp_dev->port > 0) {
			gpio_direction_output(fp_dev->port, 1);
		}
		mdelay((delay?delay:5)*RESET_TIME_MULTIPLE);
		gpio_direction_output(fp_dev->rst_port, 1);
		if (fp_dev->irq_no_use && fp_dev->port > 0) {
			gpio_direction_output(fp_dev->port, 0);
		}
...
static int silfp_irq_to_reset_init(struct silfp_data *fp_dev)
{
	...
	if (fp_dev->port > 0) {
		...
		ret = gpio_request(fp_dev->port, "SILFP_IRQ_TO_RST_PIN");
		if (ret < 0) {
			LOG_MSG_DEBUG(ERR_LOG, "[%s] Failed to request GPIO=%d, ret=%d", __func__, (s32)fp_dev->port, ret);
			return -ENODEV;
		} else {
			gpio_direction_output(fp_dev->port, 0);
		}
...
static int silfp_resource_init(struct silfp_data *fp_dev, struct fp_dev_init_t *dev_info)
{
	...
	LOG_MSG_DEBUG(INFO_LOG, "before [%s] port %d, rst_port %d.\n", __func__, fp_dev->port, fp_dev->rst_port);
	silfp_hw_poweron(fp_dev);
	msleep(5);
	...
	ret = gpio_request(fp_dev->port, "SILFP_INT_IRQ");
	if (ret < 0) {
		LOG_MSG_DEBUG(ERR_LOG, "[%s] Failed to request GPIO=%d, ret=%d", __func__, (s32)fp_dev->port, ret);
		status = -ENODEV;
		goto err;
		...
		fp_dev->irq = gpio_to_irq(fp_dev->port);
