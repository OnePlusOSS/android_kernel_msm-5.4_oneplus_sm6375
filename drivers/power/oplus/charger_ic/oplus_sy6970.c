...
int sy6970_adc_start(struct sy6970 *bq)
{
	...
	if (((val & SY6970_CONV_RATE_MASK) >> SY6970_CONV_RATE_SHIFT) ==
			SY6970_ADC_CONTINUE_ENABLE) {
...
int sy6970_adc_read_battery_volt(struct sy6970 *bq)
{
	...
	if (ret < 0) {
		...
		volt = SY6970_BATV_BASE + ((val & SY6970_BATV_MASK) >> SY6970_BATV_SHIFT) *
		       SY6970_BATV_LSB;
		return volt;
	}
}
...
int sy6970_adc_read_sys_volt(struct sy6970 *bq)
{
	...
	if (ret < 0) {
		...
		volt = SY6970_SYSV_BASE + ((val & SY6970_SYSV_MASK) >> SY6970_SYSV_SHIFT) *
		       SY6970_SYSV_LSB;
		return volt;
	}
}
...
static int _sy6970_adc_read_vbus_volt(struct sy6970 *sy)
{
	...
	if (ret < 0) {
		...
		volt = SY6970_VBUSV_BASE + ((val & SY6970_VBUSV_MASK) >> SY6970_VBUSV_SHIFT) *
		       SY6970_VBUSV_LSB;
		return volt;
	}
}
...
int sy6970_adc_read_temperature(struct sy6970 *sy)
{
	...
	if (ret < 0) {
		...
		temp = SY6970_TSPCT_BASE + ((val & SY6970_TSPCT_MASK) >> SY6970_TSPCT_SHIFT) *
		       SY6970_TSPCT_LSB;
		return temp;
	}
}
...
int sy6970_adc_read_charge_current(void)
{
	...
	if (ret < 0) {
		...
		volt = (int)(SY6970_ICHGR_BASE + ((val & SY6970_ICHGR_MASK) >>
						  SY6970_ICHGR_SHIFT) * SY6970_ICHGR_LSB);
		return volt;
	}
}
...
static int sy6970_read_idpm_limit(struct sy6970 *bq, int *icl)
{
	...
	if (ret < 0) {
		...
		*icl = SY6970_IDPM_LIM_BASE + ((val & SY6970_IDPM_LIM_MASK) >>
					       SY6970_IDPM_LIM_SHIFT) * SY6970_IDPM_LIM_LSB;
		return 0;
	}
}
...
static int sy6970_get_charger_type(struct sy6970 *bq,
...
static irqreturn_t sy6970_irq_handler(int irq, void *data)
{
	...
	if ((prev_chg_type == POWER_SUPPLY_TYPE_USB_DCP)
			&& ((cur_chg_type == POWER_SUPPLY_TYPE_USB)
			    || (cur_chg_type == POWER_SUPPLY_TYPE_USB_CDP))) {
		chg_info(" keep cur_chg_type %d as prev_chg_type %d ", cur_chg_type,
			 prev_chg_type);
	...
	if ((cur_chg_type == POWER_SUPPLY_TYPE_USB)
		...
		chg_err("oplus_chg_type = %d, vbus_type = %d", bq->oplus_chg_type,
			bq->vbus_type);
	}
...
static int oplus_chg_irq_gpio_init(struct sy6970 *sy)
{
	...
	sy->splitchg_inter_active = pinctrl_lookup_state(sy->pinctrl,
	...
	sy->splitchg_inter_sleep = pinctrl_lookup_state(sy->pinctrl,
...
static int oplus_sy6970_set_aicr(int current_ma)
{
	...
	chg_debug("usb input max current limit=%d, em_mode = %d", current_ma,
		  chip->em_mode);
	...
	chg_info("usb input max current limit=%d, aicl_point_temp=%d", current_ma,
		 aicl_point_temp);
...
int oplus_charger_type_thread(void *data)
{
	...
	RECHECK:
		...
		case SY6970_VBUS_TYPE_SDP:
			...
			if (!bq->sdp_retry) {
				bq->sdp_retry = true;
				schedule_delayed_work(&g_bq->sy6970_retry_adapter_detection,
						      OPLUS_BC12_RETRY_TIME);
			}
		...
		case SY6970_VBUS_TYPE_CDP:
			...
			if (!bq->cdp_retry) {
				bq->cdp_retry = true;
				schedule_delayed_work(&bq->sy6970_retry_adapter_detection,
						      OPLUS_BC12_RETRY_TIME_CDP);
			}
...
static void charger_type_thread_init(void)
{
	...
	charger_type_kthread = kthread_run(oplus_charger_type_thread, g_bq,
					   "chgtype_kthread");
...
static void aicr_setting_work_callback(struct work_struct *work)
{
	...
	chg_debug("begin aicr current = %d mA, wait %d mS", g_bq->aicr,
		  re_check_count * 20);
				...
				const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&bq->sy6970_retry_adapter_detection,
