...
static int __p922x_read_reg(struct oplus_p922x_ic *chip, int reg,
			    char *returnData, int count)
{
...
static int p922x_write_reg_multi_byte(struct oplus_p922x_ic *chip, int reg,
				      char *cbuf, int length)
{
	int ret;
	int send_length;
...
static int p922x_read_reg(struct oplus_p922x_ic *chip, int reg,
			  char *returnData, int count)
{
	int ret = 0;
...
static int p922x_config_interface(struct oplus_p922x_ic *chip, int RegNum,
				  int val, int MASK)
{
	char p922x_reg = 0;
	int ret = 0;
...
static int p922x_config_fan_pwm_pulse_value(struct oplus_p922x_ic *chip)
{
	...
	if (dock_version == DOCK_OAWV01) {
		chip->p922x_chg_status.dock_fan_pwm_pulse_fastchg =
			FAN_PWM_PULSE_IN_FASTCHG_MODE_V01;
		chip->p922x_chg_status.dock_fan_pwm_pulse_silent =
			FAN_PWM_PULSE_IN_SILENT_MODE_V01;

	} else if (dock_version == DOCK_OAWV02) {
		chip->p922x_chg_status.dock_fan_pwm_pulse_fastchg =
			FAN_PWM_PULSE_IN_FASTCHG_MODE_V02;
		chip->p922x_chg_status.dock_fan_pwm_pulse_silent =
			FAN_PWM_PULSE_IN_SILENT_MODE_V02;

	} else if (dock_version >= DOCK_OAWV03 && dock_version <= DOCK_OAWV07) {
		chip->p922x_chg_status.dock_fan_pwm_pulse_fastchg =
			FAN_PWM_PULSE_IN_FASTCHG_MODE_V03_07;
		chip->p922x_chg_status.dock_fan_pwm_pulse_silent =
			FAN_PWM_PULSE_IN_SILENT_MODE_V03_07;

	} else if (dock_version >= DOCK_OAWV08 && dock_version <= DOCK_OAWV15) {
		chip->p922x_chg_status.dock_fan_pwm_pulse_fastchg =
			FAN_PWM_PULSE_IN_FASTCHG_MODE_V08_15;
		chip->p922x_chg_status.dock_fan_pwm_pulse_silent =
			FAN_PWM_PULSE_IN_SILENT_MODE_V08_15;

	} else {
		chip->p922x_chg_status.dock_fan_pwm_pulse_fastchg =
			FAN_PWM_PULSE_IN_FASTCHG_MODE_DEFAULT;
		chip->p922x_chg_status.dock_fan_pwm_pulse_silent =
			FAN_PWM_PULSE_IN_SILENT_MODE_DEFAULT;
	}
...
static void p922x_set_rx_charge_current(struct oplus_p922x_ic *chip,
					int chg_current)
{
...
static void p922x_set_rx_terminate_voltage(struct oplus_p922x_ic *chip,
		int vol_value)
{
...
static int p922x_increase_vc_by_step(struct oplus_p922x_ic *chip, int src_value,
				     int limit_value, int step_value)
{
	int temp_value;
...
static int p922x_decrease_vc_by_step(struct oplus_p922x_ic *chip, int src_value,
				     int limit_value, int step_value)
{
	int temp_value = 0;
...
static int p922x_decrease_vout_to_target(struct oplus_p922x_ic *chip,
		int vout_target, int vout_min)
{
	int temp_value;
...
static int p922x_increase_vout_to_target(struct oplus_p922x_ic *chip,
		int vout_target, int vout_max)
{
	int temp_value;
...
static void p922x_charger_init(struct oplus_p922x_ic *chip)
{
	...
	if (chip->p922x_chg_status.rx_runing_mode == RX_RUNNING_MODE_EPP) {
		...
		mp2650_input_current_limit_without_aicl(
			chip->p922x_chg_status.epp_current_step);
...
void p922x_set_rtx_function(bool is_on)
{
	...
	if (is_on) {
		...
		schedule_delayed_work(&p922x_chip->idt_dischg_work,
...
void p922x_reg_print(void)
{
	...
	chg_err("<~WPC~>0x4E0 REG: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X \n",
		debug_data[0], debug_data[1], debug_data[2], debug_data[3], debug_data[4],
		debug_data[5], debug_data[6], debug_data[7],
...
static int p922x_load_bootloader(struct oplus_p922x_ic *chip)
{
	...
	rc = __p922x_write_reg(chip, 0x3048,
			       0xD0); /* map RAM address 0x1c00 to OTP 0x0000 */
...
static int p922x_load_fw(struct oplus_p922x_ic *chip, unsigned char *fw_data,
			 int CodeLength)
{
...
static int p922x_MTP(struct oplus_p922x_ic *chip, unsigned char *fw_buf,
		     int fw_size)
{
	int rc;
	int i, j;
	...
	rc = __p922x_write_reg(chip, 0x404,
			       pure_fw_size & 0xff); /* write FW length low byte */
...
static int p922x_get_vrect_iout(struct oplus_p922x_ic *chip)
{
	...
	if (chip->p922x_chg_status.charge_status ==
...
int p922x_get_CEP_flag(struct oplus_p922x_ic *chip)
{
	...
	/* -2 <= CEP <= 2 */
	if ((chip->p922x_chg_status.CEP_value == 0)
			|| (chip->p922x_chg_status.CEP_value == 1)
			|| (chip->p922x_chg_status.CEP_value == 2)
			|| (chip->p922x_chg_status.CEP_value == 0xFF)
...
{
	if ((p922x_chip->p922x_chg_status.charge_online)
			&& (chip->p922x_chg_status.CEP_ready))
...
static void p922x_ready_to_switch_to_charger(struct oplus_p922x_ic *chip,
		bool reset_ceptimeout)
{
	if (reset_ceptimeout)
...
static int p922x_fastcharge_debug_by_adb(struct oplus_p922x_ic *chip)
{
	...
	if (!chip->p922x_chg_status.iout_debug_mode
			&& !chip->p922x_chg_status.vout_debug_mode)
		return -1;
	...
	iout_max_value = chip->p922x_chg_status.iout_stated_current +
			 WPC_CHARGE_CURRENT_OFFSET;

	if (chip->p922x_chg_status.iout_stated_current > WPC_CHARGE_CURRENT_OFFSET)
		iout_min_value = chip->p922x_chg_status.iout_stated_current -
				 WPC_CHARGE_CURRENT_OFFSET;
...
#else
static void p922x_fastcharge_current_adjust_40w(struct oplus_p922x_ic *chip,
		bool is_init)
{
	...
	if ((g_oplus_chip->batt_volt >= 4500)
			&& (!chip->p922x_chg_status.wpc_ffc_charge)) {
		chg_err("<~WPC~> batt_volt[%d] >= 4.5V, Not in FFC\n", g_oplus_chip->batt_volt);
		p922x_ready_to_switch_to_charger(chip, true);
		return;

	} else if (chip->p922x_chg_status.wpc_ffc_charge
			&& (g_oplus_chip->batt_volt >= 4500)
	...
	if (chip->p922x_chg_status.wpc_reach_stable_charge) {
		if (!chip->p922x_chg_status.wpc_reach_4370mv
				&& (g_oplus_chip->batt_volt >= 4370)) {
...
ADJUST_FINISH:
	...
	if (chip->p922x_chg_status.iout_stated_current >
...
static void p922x_fastcharge_skewing_proc_40w(struct oplus_p922x_ic *chip,
		bool is_init)
{
...
static void p922x_TX_message_process(struct oplus_p922x_ic *chip)
{
	...
	if (chip->p922x_chg_status.send_msg_timer > 2) {
		...
		} else if (chip->p922x_chg_status.send_message ==
...
int p922x_charge_set_max_current_by_cool_down(struct oplus_p922x_ic *chip)
{
	...
	if (chip->p922x_chg_status.fastchg_current_limit >
			cool_down_current_limit_wireless[cool_down - 1])
		chip->p922x_chg_status.fastchg_current_limit =
			cool_down_current_limit_wireless[cool_down - 1];
...
static int p922x_charge_status_process(struct oplus_p922x_ic *chip)
{
	...
	case WPC_CHG_STATUS_DEFAULT:
		...
		if ((chip->p922x_chg_status.adapter_type == ADAPTER_TYPE_VOOC
			...
			if (g_oplus_chip->batt_volt_max > 4100)
				...
				freq_thr = chip->p922x_chg_status.freq_threshold + (g_oplus_chip->batt_volt_max
						- 3700) / 200;
	...
	case WPC_CHG_STATUS_READY_FOR_EPP:
		...
		if (chip->p922x_chg_status.epp_current_limit >=
				chip->p922x_chg_status.wpc_chg_param.epp_input_ma
				|| (chip->p922x_chg_status.epp_current_limit >=
				    chip->p922x_chg_status.wpc_chg_param.epp_temp_warm_input_ma
		...
		if (chip->p922x_chg_status.charge_type == WPC_CHARGE_TYPE_FAST) {
			if (p922x_get_CEP_flag(chip) != 0) {
				...
				if (cep_nonzero_cnt > 10) {
					...
					if (chip->p922x_chg_status.cep_info < to_cep_info(
								chip->p922x_chg_status.CEP_value))
	...
	case WPC_CHG_STATUS_INCREASE_VOLTAGE_FOR_CHARGEPUMP:
		...
		if (p922x_get_CEP_flag(chip) == 0) {
			temp_value = p922x_increase_vout_to_target(chip,
					g_oplus_chip->batt_volt * 4 + 300, WPC_CHARGE_VOLTAGE_CHGPUMP_MAX);
	...
	case WPC_CHG_STATUS_FAST_CHARGING_FROM_CHGPUMP:
		...
		if ((p922x_get_CEP_flag(chip) == 0)
			...
			if (chip->p922x_chg_status.work_silent_mode
					|| chip->p922x_chg_status.call_mode) {
				cep_nonzero_cnt = 0;
				chip->p922x_chg_status.wpc_ffc_charge = false;
	...
	case WPC_CHG_STATUS_DECREASE_IOUT_TO_200MA:
		...
		if ((p922x_get_CEP_flag(chip) == 0)
				|| chip->p922x_chg_status.wpc_skewing_proc) {
	...
	case WPC_CHG_STATUS_DECREASE_VOUT_TO_12V:
		if ((p922x_get_CEP_flag(chip) == 0)
				|| chip->p922x_chg_status.wpc_skewing_proc) {
			temp_value = p922x_decrease_vout_to_target(chip,
					WPC_CHARGE_VOLTAGE_CHGPUMP_TO_CHARGER, WPC_CHARGE_VOLTAGE_CHGPUMP_MIN);
	...
	case WPC_CHG_STATUS_ADJUST_VOL_AFTER_INC_CURRENT:
		...
		if (chip->p922x_chg_status.iout > (chip->p922x_chg_status.iout_stated_current +
						   WPC_CHARGE_CURRENT_OFFSET)) {
	...
	case WPC_CHG_STATUS_FAST_CHARGING_FROM_CHARGER:
		...
		if (g_oplus_chip->temperature >= 420) {
			...
			if (chip->p922x_chg_status.charge_current <
					chip->p922x_chg_status.fastchg_current_limit) {
		...
		if (p922x_get_CEP_flag(chip) == 0) {
			...
			if (cep_nonzero_cnt >= 3) {
				...
				} else if (chip->p922x_chg_status.charge_voltage >
						WPC_CHARGE_VOLTAGE_FASTCHG_MIN) {
					temp_value = p922x_decrease_vc_by_step(chip,
									       chip->p922x_chg_status.charge_voltage,
									       WPC_CHARGE_VOLTAGE_FASTCHG_MIN,
	...
	case WPC_CHG_STATUS_DECREASE_VOUT_FOR_RESTART:
		...
		temp_value = p922x_decrease_vout_to_target(chip,
				g_oplus_chip->batt_volt * 4 + 300, WPC_CHARGE_VOLTAGE_CHGPUMP_MIN);
...
static void p922x_idt_dischg_status(struct oplus_p922x_ic *chip)
{
	...
	if (regdata[1] != 0) {
		...
		} else if (P922X_RTX_DIGITALPING & regdata[0]
				|| P922X_RTX_ANALOGPING & regdata[0]) {
			chip->p922x_chg_status.tx_online = false;
...
static void p922x_idt_dischg_work(struct work_struct *work)
{
	...
	struct oplus_p922x_ic *chip = container_of(dwork, struct oplus_p922x_ic,
				      idt_dischg_work);
...
static void p922x_commu_data_process(struct oplus_p922x_ic *chip)
{
	...
	if (p922x_get_idt_int_val() == 0) {
		...
		schedule_delayed_work(&p922x_chip->idt_event_int_work,
...
static void p922x_idt_connect_int_func(struct work_struct *work)
{
	...
	if (p922x_get_idt_con_val() == 1) {
		...
		if (p922x_chip->p922x_chg_status.charge_online == false) {
			...
			schedule_delayed_work(&chip->p922x_task_work,
...
static void p922x_idt_int_eint_register(struct oplus_p922x_ic *chip)
{
	...
	retval = request_irq(chip->idt_int_irq, irq_idt_event_int_handler,
			     IRQF_TRIGGER_FALLING, "p922x_idt_int",
...
static void p922x_idt_con_eint_register(struct oplus_p922x_ic *chip)
{
	...
	retval = request_irq(chip->idt_con_irq, irq_idt_connect_int_handler,
			     IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "p922x_con_int",
			     chip);	/* 0X01:rising edge,0x02:falling edge */
...
static int oplus_wrx_otg_en_gpio_init(struct oplus_p922x_ic *chip)
{
	...
	chip->wrx_otg_en_active = pinctrl_lookup_state(chip->pinctrl,
				  "wrx_otg_en_active");
	...
	chip->wrx_otg_en_sleep = pinctrl_lookup_state(chip->pinctrl,
				 "wrx_otg_en_sleep");
	...
	chip->wrx_otg_en_default = pinctrl_lookup_state(chip->pinctrl,
				   "wrx_otg_en_default");
...
static int p922x_idt_gpio_init(struct oplus_p922x_ic *chip)
{
	...
	chip->ext1_wired_otg_en_gpio = of_get_named_gpio(node,
				       "qcom,ext1_wired_otg_en-gpio", 0);
	...
	chip->ext2_wireless_otg_en_gpio = of_get_named_gpio(node,
					  "qcom,ext2_wireless_otg_en-gpio", 0);
...
static ssize_t p922x_reg_store(struct file *filp, const char __user *buff,
...
static ssize_t p922x_reg_show(struct file *filp, char __user *buff,
...
static ssize_t p922x_data_log_write(struct file *filp, const char __user *buff,
				    size_t len, loff_t *data)
{
	char write_data[32] = {0};
	int critical_log = 0;
...
static ssize_t proc_wireless_ftm_mode_write(struct file *file,
		const char __user *buf, size_t len, loff_t *lo)
{
	char buffer[2] = {0};
...
static ssize_t proc_wireless_charge_pump_read(struct file *file,
		char __user *buf,
		size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
	...
	case UPGRADE_START:
		...
		if (fw_head->magic[0] == 0x02 && fw_head->magic[1] == 0x00 &&
			...
			chg_err("<IDT UPDATE>image header verification succeeded, fw_size=%d\n",
				fw_size);
			copy_from_user(fw_buf, buf + sizeof(struct idt_fw_head),
...
static ssize_t proc_wireless_user_sleep_mode_read(struct file *file,
		char __user *buf,
		size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
...
static ssize_t proc_wireless_user_sleep_mode_write(struct file *file,
		const char __user *buf,
		size_t len, loff_t *lo)
{
	...
	if (pmw_pulse == FASTCHG_MODE) {
		...
		rc = p922x_set_dock_fan_pwm_pulse(
			     chip->p922x_chg_status.dock_fan_pwm_pulse_silent);
...
static ssize_t proc_wireless_idt_adc_test_read(struct file *file,
		char __user *buf,
		size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
...
static ssize_t proc_wireless_idt_adc_test_write(struct file *file,
		const char __user *buf,
		size_t len, loff_t *lo)
{
	char buffer[4] = {0};
