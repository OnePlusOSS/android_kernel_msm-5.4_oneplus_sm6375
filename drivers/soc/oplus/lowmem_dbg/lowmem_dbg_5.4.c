...
static int dump_slab(bool verbose)
{
	if (likely(!verbose)) {
		...
		if (max_cachep || prev_max_cachep) {
			pr_info("name            <active_objs> <num_objs> <objsize> <objperslab> <pagesperslab> : tunables <limit> <batchcount> ");
			pr_info("<sharedfactor> : slabdata <active_slabs> <num_slabs> <sharedavail>\n");
		}
